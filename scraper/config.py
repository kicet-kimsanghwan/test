...
# '센텀시티 구내식당 식단표' 목록 카테고리 번호(고정). 이름 검색보다 우선 사용.
NAVER_CATEGORY_NO = "9"
# 카테고리 이름(부분 일치). 번호를 못 쓸 때 이름으로 찾는 폴백.
...
KAKAO_RECENT_DAYS = 2
# 당일 게시물들에서 식단표 후보로 살펴볼 이미지 최대 개수(이 중 1장만 최종 선택).
# 정담은 대표메뉴 사진첩(여러 장)+식단표가 함께 올라오므로 넉넉히.
KAKAO_CANDIDATES = 24
# 한 게시물에서 가져올 후보 수(앨범 게시물이 후보를 독점해 식단표 게시물이
# 누락되는 것을 방지). 카카오는 같은 이미지를 여러 해상도로 주므로 넉넉히.
KAKAO_PER_POST = 8
